...
public class OSCConnection extends Connection {
	...
	public void send(SocketCommand sm) {
		if (sm.command == SocketCommand.COMMAND_MIDI_SHORT_MESSAGE) {
			...
			OSCMessage msg = null;
			if (msc.midiCommand == 0xB0) {
				Object args[] = new Object[1];
				args[0] = new Integer(msc.data2);
				msg = new OSCMessage("/fingerplay/control/" + msc.data1, args);
			} else if (msc.midiCommand == 0x90) {
				Object args[] = new Object[2];
				args[0] = new Integer(msc.data1);
				args[1] = new Integer(msc.data2);
				msg = new OSCMessage("/fingerplay/noteon", args);
			} else if (msc.midiCommand == 0x80) {
				Object args[] = new Object[2];
				args[0] = new Integer(msc.data1);
				args[1] = new Integer(msc.data2);
				msg = new OSCMessage("/fingerplay/noteoff", args);
			}
			if (msg != null) {
				try {
